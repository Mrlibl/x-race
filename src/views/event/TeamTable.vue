...
import DrawerTeamInfo from '@/views/campaign/DrawerTeamInfo.vue'
...
watch(() => options.value.page, (newPage) => {
  getTeamData(route.query.id)
})

const getTeamData = async (id: any) => {
  try {
    const url = `https://nodeapi.carx.cz/lottery/getRacingTeams`
    const payload = {
      match_id: id,
      ...
      <VTable class="text-no-wrap team-members-table" style="width: 100%;">
        ...
        <tbody>
          <tr v-for="(item, index) in tableList" :key="index" style="cursor: pointer;" @click="infoClick(item)">
            ...
            <td>
              <div class="d-flex align-center ms-3" style="font-size: 0.875rem; color: #fff;">
                <img :src="item?.icon" style="width: 1.875rem;" />
              </div>
            </td>
