...
const clickMenu = async (item) => {
  ...
  if (to === "token-analysis") {
    const url = new URL(window.location.href)
    window.history.replaceState({}, '', url.origin + '/tokenAnalysis')
    document.dispatchEvent(new CustomEvent('initToken'))
  }
}
</script>
  ...
  <Component :is="props.tag" ref="refNav" class="layout-vertical-nav" :class="[
    ...
    <div class="nav-header">
      <slot name="nav-header">
        ...
        <a href="https://carx.cz/" class="app-logo app-title-wrapper">
          <VNodeRenderer :nodes="layoutConfig.app.logo" />
        </a>
    ...
    <slot name="nav-items" :update-is-vertical-nav-scrolled="updateIsVerticalNavScrolled">
      <PerfectScrollbar :key="configStore.isAppRTL" tag="ul" class="nav-items" :options="{ wheelPropagation: false }"
        ...
        <Component @click="clickMenu(item)" :is="resolveNavItemComponent(item)" v-for="(item, index) in navItems"
          :key="index" :item="item" />
      </PerfectScrollbar>
    </slot>
