...
const wasToggledManually = ref(false)
const toggleGroup = () => {
  isGroupOpen.value = !isGroupOpen.value
  wasToggledManually.value = true
}
/**
...
watch(
  ...
  () => {
    ...
    // 仅在没有手动控制过时才自动设置展开
    if (!wasToggledManually.value) {
      isGroupOpen.value = (
        (!configStore.isVerticalNavMini(isVerticalNavHovered).value && isActive) ||
